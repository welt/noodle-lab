...
const isValidGitHubUrl = (url) =>
  typeof url === "string" && /^https:\/\/github\.com\/.+/.test(url);

export default class GitHubLinkButton extends HTMLElement {
  constructor() {
    super();
  }

  connectedCallback() {
    // Cascade: attribute first, then injected constant.
    let repoUrl = this.getAttribute("repo-url");
    if (!isValidGitHubUrl(repoUrl)) {
      repoUrl = isValidGitHubUrl(GITHUB_REPO_URL) ? GITHUB_REPO_URL : "";
    }
